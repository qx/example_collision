...
//        stars.push(star);
//    }
    planemap = new createjs.Bitmap(SHELTER);
    var hitArea = new createjs.Shape();
    hitArea.graphics.beginFill("#FFF").drawEllipse(-11, -24, 20, 55);
    // position hitArea relative to the internal coordinate system of the target bitmap instances:
    hitArea.x = planemap.width / 2;
    hitArea.y = planemap.height / 2;

    planemap.hitArea = hitArea;
    stage.addChild(planemap);
    ...
    shelter.rotation = -180;
    stage.addChild(shelter);
    planemap.addEventListener("mousedown", function (evt) {
    ...
    planemap.addEventListener("pressmove", function (evt) {
        var o = evt.target;
...
//        star.rotation += star.rotVel;
    ...
    var intersection = collisionMethod(shelter, planemap, window.alphaThresh);
    if (intersection) {
